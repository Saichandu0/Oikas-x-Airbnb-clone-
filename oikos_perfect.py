...
# Database functions
def init_database():
    conn = sqlite3.connect('oikos_perfect.db')
    cursor = conn.cursor()
...
# Authentication page
def show_auth_page():
    # Beach background with floating elements
    st.markdown("""
    <style>
    ...
    st.markdown("""
    <div style="position: fixed; top: 10%; left: 10%; font-size: 3rem; opacity: 0.6; z-index: 1;">✈️</div>
    <div style="position: fixed; top: 20%; right: 15%; font-size: 2.5rem; opacity: 0.6; z-index: 1;">📷</div>
    <div style="position: fixed; bottom: 20%; left: 20%; font-size: 2.8rem; opacity: 0.6; z-index: 1;">🧭</div>
    <div style="position: fixed; bottom: 15%; right: 10%; font-size: 2.2rem; opacity: 0.6; z-index: 1;">🧳</div>
    """, unsafe_allow_html=True)
...
# Main application
def show_main_app():
    # Beautiful header with centered Oikos
    st.markdown("""
    <div style="background: linear-gradient(135deg, #3498db, #1abc9c); padding: 4rem 2rem; margin: -1rem -1rem 2rem -1rem; border-radius: 0 0 30px 30px; text-align: center; position: relative; overflow: hidden;">
    ...
    <div style="position: fixed; top: 20px; left: 20px; z-index: 1000;">
        <div style="background: linear-gradient(135deg, #3498db, #1abc9c); border-radius: 15px; padding: 15px; box-shadow: 0 8px 25px rgba(52, 152, 219, 0.3); cursor: pointer;">
            <div style="width: 25px; height: 3px; background: white; margin: 5px 0; border-radius: 2px;"></div>
            <div style="width: 25px; height: 3px; background: white; margin: 5px 0; border-radius: 2px;"></div>
            <div style="width: 25px; height: 3px; background: white; margin: 5px 0; border-radius: 2px;"></div>
        </div>
    </div>
    ...
    # Hidden toggle button (functionality only)
    col1, col2, col3, col4, col5 = st.columns([1, 1, 1, 1, 1])
    with col1:
        # Make button invisible but functional
        if st.button("☰", key="toggle_sidebar", help="Open Navigation Menu"):
...
def show_payment_page():
    ...
    # Navigation buttons after successful booking
